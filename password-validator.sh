...
#Variables to store color codes
green='\033[0;32m'
red='\033[0;31m'
clear='\033[0m'
				...
				if test $? -eq -0; then
					...
					echo "$password" | grep -q [a-z]
						if test $? -eq 0; then					
							...
							echo -e  "${red}password lenght valid + contains number + capital BUT NO small alphabets ${clear}" >&2
							exit 1
							echo "Exit Code is : " $?				
						fi
				else
					echo -e "${red} password length valid + contains number + Small alphahets BUT no capital alphabets ${clear}"
					#exit 1
					#echo "Exit code is :" $?
				fi
		else
			echo -e "${red}password length valid + BUT no numbers AND small AND capital alpbahets ${clear} "
			#exit 1
			#echo "Exit Code is : " $? 
